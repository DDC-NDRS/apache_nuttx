 ...
 *
 ****************************************************************************/
int /**/nxsig_timedwait(FAR sigset_t const* set, FAR struct siginfo* info, 
    ...
    /* We will have to wait for a signal to be posted to this task. */
        ...
        if (timeout != NULL) {
            ...
            waitticks = ((uint64_t)timeout->tv_sec * NSEC_PER_SEC + (uint64_t)timeout->tv_nsec + NSEC_PER_TICK - 1) /
                        NSEC_PER_TICK;
            #else
            uint32_t waitmsec;
            ...
            if (waitticks > 0) {
                ...
                /* Start the watchdog */
                wd_start(&rtcb->waitdog, waitticks, nxsig_timeout, (uintptr_t)rtcb);
 ...
 *
 ****************************************************************************/
int sigtimedwait(FAR sigset_t const* set, FAR struct siginfo* info, 
                 FAR const struct timespec* timeout) {
    int ret;
