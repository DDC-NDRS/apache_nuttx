 ...
 *
 ****************************************************************************/
int /**/nxsig_action(int signo, FAR const struct sigaction* act, FAR struct sigaction* oact, bool force) {
     ...
     * caught or ignored.
     */
    if (act != NULL && !force && act->sa_handler != SIG_DFL && !nxsig_iscatchable(signo)) {
        return (-EINVAL);
    }
    #endif
