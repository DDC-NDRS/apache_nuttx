...
#define CALL_VECTOR(ndx, vector, irq, context, arg) \
    do {                                    \
        ...
        if (CONFIG_SCHED_CRITMONITOR_MAXTIME_IRQ > 0 && elapsed > CONFIG_SCHED_CRITMONITOR_MAXTIME_IRQ) { \
            serr("IRQ %d(%p), execute time too long %" PRIu32 "\n", irq, vector, elapsed);  \
        }                                   \
    } while (0)
#else
#define CALL_VECTOR(ndx, vector, irq, context, arg) vector(irq, context, arg)
...
void /**/irq_dispatch(int irq, FAR void* context) {
    ...
    #ifdef CONFIG_DEBUG_MM
    if ((rtcb->flags & TCB_FLAG_HEAP_CHECK) || (this_task()->flags & TCB_FLAG_HEAP_CHECK)) {
        kmm_checkcorruption();
    }
    #endif
