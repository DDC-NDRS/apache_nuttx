...
static int nxmq_file_poll(FAR struct file* filep, struct pollfd* fds, bool setup);
...
static int file_mq_vopen(FAR struct file* mq, FAR char const* mq_name, int oflags, mode_t umask, va_list ap,
                         int* created) {
    ...
    if (sizeof(CONFIG_FS_MQUEUE_VFS_PATH) + 1 + strlen(mq_name) >= MAX_MQUEUE_PATH) {
        ret = -ENAMETOOLONG;
        goto errout;
    }
...
static mqd_t nxmq_vopen(FAR char const* mq_name, int oflags, va_list ap) {
    struct file mq;
    int created;
    int ret;
    ...
    ret = file_allocate(mq.f_inode, mq.f_oflags, mq.f_pos, mq.f_priv, 0, false);
    if (ret < 0) {
        file_mq_close(&mq);
        if (created) {
 ...
 *
 ****************************************************************************/
int file_mq_open(FAR struct file* mq, FAR char const* mq_name, int oflags, ...) {
    va_list ap;
    int     ret;
 ...
 *
 ****************************************************************************/
mqd_t /**/nxmq_open(FAR char const* mq_name, int oflags, ...) {
    va_list ap;
    mqd_t   ret;
 ...
 *
 ****************************************************************************/
mqd_t /**/mq_open(FAR char const* mq_name, int oflags, ...) {
    va_list ap;
    mqd_t   ret;
