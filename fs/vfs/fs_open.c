 ...
 *
 ****************************************************************************/
static int file_vopen(FAR struct file* filep, FAR char const* path, 
    ...
    #if defined(CONFIG_BCH) && !defined(CONFIG_DISABLE_MOUNTPOINT) && !defined(CONFIG_DISABLE_PSEUDOFS_OPERATIONS)
 ...
 *
 ****************************************************************************/
static int nx_vopen(FAR char const* path, int oflags, va_list ap) {
    struct file filep;
    int ret;
    int fd;
    ...
    fd = file_allocate(filep.f_inode, filep.f_oflags, filep.f_pos, filep.f_priv, 0, false);
    if (fd < 0) {
        file_close(&filep);
        return fd;
    }
 ...
 *
 ****************************************************************************/
int /**/file_open(FAR struct file* filep, FAR char const* path, int oflags, ...) {
    va_list ap;
    int     ret;
 ...
 *
 ****************************************************************************/
int nx_open(FAR char const* path, int oflags, ...) {
    va_list ap;
    int     fd;
 ...
 *
 ****************************************************************************/
int /**/open(FAR char const* path, int oflags, ...) {
    va_list ap;
    int fd;
