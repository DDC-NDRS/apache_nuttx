...
private:
  ...
  static constexpr bool check_range() {
    ...
    return max_seconds >= numeric_limits<TimeT>::max() &&
           min_seconds <= (const rep)numeric_limits<TimeT>::min();
  }
...
public:
  ...
  static _LIBCPP_CONSTEXPR_AFTER_CXX11 bool checked_set(CType* out,
    ...
    if (time > (ChronoType)Lim::max() || time < (ChronoType)Lim::min())
      return false;
  ...
  static _LIBCPP_CONSTEXPR_AFTER_CXX11 bool is_representable(TimeSpecT tm) {
    ...
    } else if (tm.tv_sec == (time_t)(min_seconds - 1)) {
      return tm.tv_nsec >= min_nsec_timespec;
    } else {
      return tm.tv_sec >= (time_t)min_seconds;
    }
  }
  ...
  static _LIBCPP_CONSTEXPR_AFTER_CXX11 bool is_representable(FileTimeT tm) {
    ...
    if (secs.count() >= 0)
      return ((TimeT)secs.count() <= TLim::max());
    return ((TimeT)secs.count() >= TLim::min());
  }
