...
#define RXDMA_BUFFER_SIZE ((CONFIG_STM32H7_SERIAL_RXDMA_BUFFER_SIZE + RXDMA_BUFFER_MASK) & ~RXDMA_BUFFER_MASK)
...
#define SERIAL_RXDMA_CONTROL_WORD                                                                                      \
    (DMA_SCR_DIR_P2M | DMA_SCR_CIRC | DMA_SCR_MINC | DMA_SCR_PSIZE_8BITS | DMA_SCR_MSIZE_8BITS |                       \
     CONFIG_USART_RXDMAPRIO | DMA_SCR_PBURST_SINGLE | DMA_SCR_MBURST_SINGLE)
...
#define TXDMA_BUFFER_SIZE ((CONFIG_STM32H7_SERIAL_RXDMA_BUFFER_SIZE + RXDMA_BUFFER_MASK) & ~RXDMA_BUFFER_MASK)
...
#define SERIAL_TXDMA_CONTROL_WORD                                                                                      \
    (DMA_SCR_DIR_M2P | DMA_SCR_MINC | DMA_SCR_PSIZE_8BITS | DMA_SCR_MSIZE_8BITS | DMA_SCR_PBURST_SINGLE |              \
     DMA_SCR_MBURST_SINGLE | CONFIG_USART_TXDMAPRIO | DMA_SCR_TRBUFF)
...
#if defined(CONFIG_STM32H7_FLOWCONTROL_BROKEN) && !defined(CONFIG_SERIAL_IFLOWCONTROL_WATERMARKS)
...
struct up_dev_s {
    ...
    bool const     stopbits2; /* True: Configure with 2 stop bits instead of 1 */

    #ifdef CONFIG_SERIAL_IFLOWCONTROL
    bool const     iflow;     /* input flow control (RTS) enabled */
    #endif

    #ifdef CONFIG_SERIAL_OFLOWCONTROL
    bool const     oflow;     /* output flow control (CTS) enabled */
    #endif
    ...
    /* TX DMA state */
    #ifdef SERIAL_HAVE_TXDMA
    unsigned int const txdma_channel; /* DMA channel assigned */
    ...
    /* RX DMA state */
    #ifdef SERIAL_HAVE_RXDMA
    unsigned int const rxdma_channel; /* DMA channel assigned */
    ...
    bool const     rs485_dir_polarity; /* U[S]ART RS-485 DIR pin state for TX enabled */
    #endif
};
...
static char g_usart1rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV7M_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_USART2_RXDMA
static char g_usart2rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV7M_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_USART3_RXDMA
static char g_usart3rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV7M_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_UART4_RXDMA
static char g_uart4rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV7M_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_UART5_RXDMA
static char g_uart5rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV7M_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_USART6_RXDMA
static char g_usart6rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV7M_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_UART7_RXDMA
static char g_uart7rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV7M_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_UART8_RXDMA
static char g_uart8rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV7M_DCACHE_LINESIZE);
#endif

/* Receive/Transmit buffers */
#ifdef CONFIG_STM32H7_USART1
static char g_usart1rxbuffer[CONFIG_USART1_RXBUFSIZE];
static char g_usart1txbuffer[USART1_TXBUFSIZE_ADJUSTED] USART1_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_STM32H7_USART2
static char g_usart2rxbuffer[CONFIG_USART2_RXBUFSIZE];
static char g_usart2txbuffer[USART2_TXBUFSIZE_ADJUSTED] USART2_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_STM32H7_USART3
static char g_usart3rxbuffer[CONFIG_USART3_RXBUFSIZE];
static char g_usart3txbuffer[USART3_TXBUFSIZE_ADJUSTED] USART3_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_STM32H7_UART4
static char g_uart4rxbuffer[CONFIG_UART4_RXBUFSIZE];
static char g_uart4txbuffer[UART4_TXBUFSIZE_ADJUSTED] UART4_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_STM32H7_UART5
static char g_uart5rxbuffer[CONFIG_UART5_RXBUFSIZE];
static char g_uart5txbuffer[UART5_TXBUFSIZE_ADJUSTED] UART5_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_STM32H7_USART6
static char g_usart6rxbuffer[CONFIG_USART6_RXBUFSIZE];
static char g_usart6txbuffer[USART6_TXBUFSIZE_ADJUSTED] USART6_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_STM32H7_UART7
static char g_uart7rxbuffer[CONFIG_UART7_RXBUFSIZE];
static char g_uart7txbuffer[UART7_TXBUFSIZE_ADJUSTED] UART7_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_STM32H7_UART8
static char g_uart8rxbuffer[CONFIG_UART8_RXBUFSIZE];
static char g_uart8txbuffer[UART8_TXBUFSIZE_ADJUSTED] UART8_TXBUFSIZE_ALGN;
#endif
...
static struct pm_config_s g_serialpm = {
    .pm_cb.notify = up_pm_notify, .pm_cb.prepare = up_pm_prepare, .serial_suspended = false};
#endif
...
#if !defined(SERIAL_HAVE_ONLY_DMA) || defined(CONFIG_PM) || defined(HAVE_RS485)
...
#if defined(CONFIG_SERIAL_IFLOWCONTROL) && !defined(CONFIG_STM32H7_FLOWCONTROL_BROKEN)
...
static void up_setsuspend(struct uart_dev_s* dev, bool suspend) {
    ...
    if (suspend) {
        ...
        if (priv->iflow) {
            ...
            stm32_configgpio((priv->rts_gpio & ~GPIO_MODE_MASK) | (GPIO_OUTPUT | GPIO_OUTPUT_SET));
        }
#endif
...
static int /**/up_setup(struct uart_dev_s* dev) {
    ...
    regval = up_serialin(priv, STM32_USART_CR2_OFFSET);
    regval &=
        ~(USART_CR2_STOP_MASK | USART_CR2_CLKEN | USART_CR2_CPOL | USART_CR2_CPHA | USART_CR2_LBCL | USART_CR2_LBDIE);
...
static int up_interrupt(int irq, void* context, void* arg) {
    ...
    if ((priv->sr & USART_ISR_TC) != 0 && (priv->ie & USART_CR1_TCIE) != 0 && (priv->ie & USART_CR1_TXEIE) == 0) {
        stm32_gpiowrite(priv->rs485_dir_gpio, !priv->rs485_dir_polarity);
...
static int up_ioctl(struct file* filep, int cmd, unsigned long arg) {
    ...
    defined(CONFIG_STM32H7_USART_SINGLEWIRE) || defined(CONFIG_STM32H7_SERIALBRK_BSDCOMPAT)
    struct inode*      inode = filep->f_inode;
    struct uart_dev_s* dev   = inode->i_private;
#endif
#if defined(CONFIG_SERIAL_TERMIOS) || defined(CONFIG_STM32H7_USART_SINGLEWIRE) ||                                      \
    ...
    switch (cmd) {
        ...
        case TIOCSSINGLEWIRE : {
            ...
            if ((arg & SER_SINGLEWIRE_ENABLED) != 0) {
                uint32_t gpio_val =
                    (arg & SER_SINGLEWIRE_PUSHPULL) == SER_SINGLEWIRE_PUSHPULL ? GPIO_PUSHPULL : GPIO_OPENDRAIN;
...
#if defined(CONFIG_SERIAL_IFLOWCONTROL_WATERMARKS) && defined(CONFIG_STM32H7_FLOWCONTROL_BROKEN)
    if (priv->iflow && (priv->rts_gpio != 0)) {
...
static void up_dma_send(struct uart_dev_s* dev) {
    ...
    up_clean_dcache((uintptr_t)dev->dmatx.buffer, (uintptr_t)dev->dmatx.buffer + dev->dmatx.length);
    ...
    if (dev->dmatx.nbuffer) {
        ...
        up_clean_dcache((uintptr_t)dev->dmatx.nbuffer, (uintptr_t)dev->dmatx.nbuffer + dev->dmatx.nlength);
    }
 ...
 *
 ****************************************************************************/
// @see nuttx/arch/arm/src/common/arm_initialize.c
...
struct device* arm_get_serial_dev(uint8_t uart_no) {
    void* ret;
