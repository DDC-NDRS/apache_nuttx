...
struct stm32_dma_ops_s {
    ...
    void (*dma_start)(DMA_HANDLE handle, dma_callback_t callback, void* arg, bool half);
    ...
    void (*dma_dump)(DMA_HANDLE handle, char const* msg);
};
...
static void   stm32_mdma_start(DMA_HANDLE handle, dma_callback_t callback, void* arg, bool half);
...
static void stm32_mdma_dump(DMA_HANDLE handle, char const* msg);
#endif
#endif
...
static void   stm32_sdma_start(DMA_HANDLE handle, dma_callback_t callback, void* arg, bool half);
static size_t stm32_sdma_residual(DMA_HANDLE handle);
#ifdef CONFIG_STM32H7_DMACAPABLE
static bool stm32_sdma_capable(stm32_dmacfg_t* cfg);
#endif
#ifdef CONFIG_DEBUG_DMA_INFO
static void stm32_sdma_dump(DMA_HANDLE handle, char const* msg);
#endif
#endif
...
static void   stm32_bdma_start(DMA_HANDLE handle, dma_callback_t callback, void* arg, bool half);
static size_t stm32_bdma_residual(DMA_HANDLE handle);
#ifdef CONFIG_STM32H7_DMACAPABLE
static bool stm32_bdma_capable(stm32_dmacfg_t* cfg);
#endif
#ifdef CONFIG_DEBUG_DMA_INFO
static void stm32_bdma_dump(DMA_HANDLE handle, char const* msg);
#endif
#endif
...
static void        stm32_gdma_limits_get(uint8_t controller, uint8_t* first, uint8_t* last);

static inline void dmachan_modifyreg32(DMA_CHANNEL dmachan, uint32_t offset, uint32_t clrbits, uint32_t setbits) {
...
static void stm32_gdma_limits_get(uint8_t controller, uint8_t* first, uint8_t* nchan) {
...
static void /**/ stm32_sdma_setup(DMA_HANDLE handle, stm32_dmacfg_t* cfg) {
    ...
    if ((dmachan_getreg(dmachan, STM32_DMA_SCR_OFFSET) & DMA_SCR_EN) != 0) {
        ...
        timeout = 100;
        while (timeout != 0 && (dmachan_getreg(dmachan, STM32_DMA_SCR_OFFSET) & DMA_SCR_EN) != 0) {
            up_udelay(1);
            timeout--;
        }

        DEBUGASSERT(timeout != 0 && (dmachan_getreg(dmachan, STM32_DMA_SCR_OFFSET) & DMA_SCR_EN) == 0);
    }
    ...
    regval = dmachan_getreg(dmachan, STM32_DMA_SCR_OFFSET);
    regval &=
        ~(DMA_SCR_PFCTRL | DMA_SCR_DIR_MASK | DMA_SCR_PINC | DMA_SCR_MINC | DMA_SCR_PSIZE_MASK | DMA_SCR_MSIZE_MASK |
          DMA_SCR_PINCOS | DMA_SCR_CIRC | DMA_SCR_DBM | DMA_SCR_CT | DMA_SCR_PBURST_MASK | DMA_SCR_MBURST_MASK);
    scr &= (DMA_SCR_PFCTRL | DMA_SCR_DIR_MASK | DMA_SCR_PINC | DMA_SCR_MINC | DMA_SCR_PSIZE_MASK | DMA_SCR_MSIZE_MASK |
            DMA_SCR_PINCOS | DMA_SCR_DBM | DMA_SCR_CIRC | DMA_SCR_PBURST_MASK | DMA_SCR_MBURST_MASK | DMA_SCR_TRBUFF);
    regval |= scr;
    dmachan_putreg(dmachan, STM32_DMA_SCR_OFFSET, regval);
}
...
static bool stm32_sdma_capable(stm32_dmacfg_t* cfg) {
    ...
    if ((ccr & DMA_SCR_DIR_MASK) == DMA_SCR_DIR_P2M || (ccr & DMA_SCR_DIR_MASK) == DMA_SCR_DIR_M2M) {
        if ((cfg->maddr & (ARMV7M_DCACHE_LINESIZE - 1)) != 0 || ((mend + 1) & (ARMV7M_DCACHE_LINESIZE - 1)) != 0) {
    ...
    switch (cfg->maddr & STM32_REGION_MASK) {
        ...
        case STM32_SRAM_BASE : {
            ...
            if (cfg->maddr >= STM32_DTCRAM_BASE && (cfg->maddr - STM32_DTCRAM_BASE) < 0x1ffff) {
        ...
        case STM32_CODE_BASE : {
            ...
            if (cfg->maddr >= STM32_ITCM_BASE && (cfg->maddr - STM32_ITCM_BASE) < 0xffff) {
...
static void stm32_sdma_dump(DMA_HANDLE handle, char const* msg) {
    ...
    stm32_dmamux_dump(g_dma[dmachan->ctrl].dmamux, dmachan->chan + g_dma[dmachan->ctrl].dmamux_offset);
}
#endif
...
static void stm32_bdma_disable(DMA_CHANNEL dmachan) {
    ...
    dmabase_putreg(dmachan, STM32_BDMA_IFCR_OFFSET, (BDMA_CHAN_MASK << dmachan->shift));
}
...
static int stm32_bdma_interrupt(int irq, void* context, void* arg) {
    ...
    status = (dmabase_getreg(dmachan, STM32_BDMA_ISR_OFFSET) >> dmachan->shift) & BDMA_CHAN_MASK;
...
static inline int32_t stm32_sdma_scr_2_bdma_ccr(int32_t scr) {
    ...
    ccr |= (scr & DMA_SCR_MSIZE_MASK) >> (DMA_SCR_MSIZE_SHIFT - BDMA_CCR_MSIZE_SHIFT);
    ccr |= (scr & DMA_SCR_PSIZE_MASK) >> (DMA_SCR_PSIZE_SHIFT - BDMA_CCR_PSIZE_SHIFT);
...
static void stm32_bdma_setup(DMA_HANDLE handle, stm32_dmacfg_t* cfg) {
    ...
    if ((dmachan_getreg(dmachan, STM32_BDMACH_CCR_OFFSET) & BDMA_CCR_EN) != 0) {
        ...
        timeout = 100;
        while (timeout != 0 && (dmachan_getreg(dmachan, STM32_BDMACH_CCR_OFFSET) & BDMA_CCR_EN) != 0) {
            up_udelay(1);
            timeout--;
        }

        DEBUGASSERT(timeout != 0 && (dmachan_getreg(dmachan, STM32_BDMACH_CCR_OFFSET) & BDMA_CCR_EN) == 0);
    }
    ...
    if (scr & DMA_SCR_DBM) {
        dmachan_putreg(dmachan, STM32_BDMACH_CM1AR_OFFSET, cfg->maddr + cfg->ndata);
    }
    ...
    regval = dmachan_getreg(dmachan, STM32_BDMACH_CCR_OFFSET);
    regval &= ~(BDMA_CCR_DIR | BDMA_CCR_CIRC | BDMA_CCR_PINC | BDMA_CCR_MINC | BDMA_CCR_PSIZE_MASK |
                BDMA_CCR_MSIZE_MASK | BDMA_CCR_PL_MASK | BDMA_CCR_M2M | BDMA_CCR_DBM | BDMA_CCR_CT);
    ccr = stm32_sdma_scr_2_bdma_ccr(scr);
    ccr &= (BDMA_CCR_DIR | BDMA_CCR_CIRC | BDMA_CCR_PINC | BDMA_CCR_MINC | BDMA_CCR_PSIZE_MASK | BDMA_CCR_MSIZE_MASK |
...
static void stm32_bdma_start(DMA_HANDLE handle, dma_callback_t callback, void* arg, bool half) {
    ...
    if ((ccr & (BDMA_CCR_DBM | BDMA_CCR_CIRC)) == 0) {
        ...
        ccr |= (half ? (BDMA_CCR_HTIE | BDMA_CCR_TEIE) : (BDMA_CCR_TCIE | BDMA_CCR_TEIE));
    }
    else {
        /* In non-stop modes, when the transfer completes it immediately resets
         * and starts again.  The transfer-complete interrupt is thus always
...
void weak_function arm_dma_initialize(void) {
    ...
    for (channel = 0; channel < DMA_NCHANNELS; channel++) {
        ...
        irq_attach(dmachan->irq, g_dma_ops[controller].dma_interrupt, dmachan);
...
#ifdef CONFIG_DEBUG_DMA_INFO
void stm32_dmadump(DMA_HANDLE handle, char const* msg) {
    DMA_CHANNEL dmachan    = (DMA_CHANNEL)handle;
    uint8_t     controller = dmachan->ctrl;
