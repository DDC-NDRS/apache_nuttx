 ...
 ****************************************************************************/
#ifdef HAVE_LPUART_DEVICE
int s32k1xx_lpuart_configure(uint32_t base, const struct uart_config_s* config) {
...
void s32k1xx_lowputc(int ch) {
    while ((getreg32(S32K1XX_CONSOLE_BASE + S32K1XX_LPUART_STAT_OFFSET) & LPUART_STAT_TDRE) == 0) {
        /* pass */
    }
