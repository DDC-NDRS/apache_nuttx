...
#define CONFIG_S32K1XX_I2CTIMEOTICKS  (SEC2TICK(CONFIG_S32K1XX_I2CTIMEOSEC) + MSEC2TICK(CONFIG_S32K1XX_I2CTIMEOMS))
#endif
 ...
 ****************************************************************************/
static inline uint32_t s32k1xx_lpi2c_getreg(struct s32k1xx_lpi2c_priv_s* priv, uint16_t offset);
static inline void     s32k1xx_lpi2c_putreg(struct s32k1xx_lpi2c_priv_s* priv, uint16_t offset, uint32_t value);
static inline void     s32k1xx_lpi2c_modifyreg(struct s32k1xx_lpi2c_priv_s* priv, uint16_t offset, uint32_t clearbits,
                                               uint32_t setbits);
...
static inline int s32k1xx_lpi2c_sem_waitdone(struct s32k1xx_lpi2c_priv_s* priv);

#ifdef CONFIG_I2C_TRACE
static void s32k1xx_lpi2c_tracereset(struct s32k1xx_lpi2c_priv_s* priv);
static void s32k1xx_lpi2c_tracenew(struct s32k1xx_lpi2c_priv_s* priv, uint32_t status);
static void s32k1xx_lpi2c_traceevent(struct s32k1xx_lpi2c_priv_s* priv, enum s32k1xx_trace_e event, uint32_t parm);
...
static void            s32k1xx_lpi2c_setclock(struct s32k1xx_lpi2c_priv_s* priv, uint32_t frequency);
static inline void     s32k1xx_lpi2c_sendstart(struct s32k1xx_lpi2c_priv_s* priv, uint8_t address);
static inline void     s32k1xx_lpi2c_sendstop(struct s32k1xx_lpi2c_priv_s* priv);
static inline uint32_t s32k1xx_lpi2c_getstatus(struct s32k1xx_lpi2c_priv_s* priv);
...
static int s32k1xx_lpi2c_transfer(struct i2c_master_s* dev, struct i2c_msg_s* msgs, int count);
#ifdef CONFIG_I2C_RESET
static int s32k1xx_lpi2c_reset(struct i2c_master_s* dev);
#endif

#ifdef CONFIG_S32K1XX_LPI2C_DMA
static void s32k1xx_rxdma_callback(DMACH_HANDLE handle, void* arg, bool done, int result);
static void s32k1xx_txdma_callback(DMACH_HANDLE handle, void* arg, bool done, int result);
#endif
...
#ifdef CONFIG_I2C_TRACE
static char const* g_trace_names[] = {"NONE           ", "SENDADDR       ", "SENDBYTE       ", "RCVBYTE        ",
                                      "NOSTART        ", "START/RESTART  ", "STOP           ", "ERROR          "};
#endif
...
static inline int s32k1xx_lpi2c_sem_waitdone(struct s32k1xx_lpi2c_priv_s* priv) {
    ...
    do {
        ...
        #ifdef CONFIG_S32K1XX_I2C_DYNTIMEO
        ret = nxsem_tickwait_uninterruptible(&priv->sem_isr, s32k1xx_lpi2c_toticks(priv->msgc, priv->msgv));
        #else
        ret = nxsem_tickwait_uninterruptible(&priv->sem_isr, CONFIG_S32K1XX_I2CTIMEOTICKS);
        #endif
...
static void s32k1xx_rxdma_callback(DMACH_HANDLE handle, void* arg, bool done, int result) {
    ...
    s32k1xx_lpi2c_modifyreg(priv, S32K1XX_LPI2C_MIER_OFFSET, 0, LPI2C_MIER_SDIE);
    ...
    if (result != OK) {
        ...
        if ((priv->status & LPI2C_MSR_ERROR_MASK) != 0) {
            ...
            s32k1xx_lpi2c_modifyreg(priv, S32K1XX_LPI2C_MCR_OFFSET, 0, LPI2C_MCR_RTF | LPI2C_MCR_RRF);

            /* Clear the error */
            s32k1xx_lpi2c_putreg(priv, S32K1XX_LPI2C_MSR_OFFSET,
                                 (priv->status & (LPI2C_MSR_NDF | LPI2C_MSR_ALF | LPI2C_MSR_FEF | LPI2C_MSR_PLTF)));
...
static void s32k1xx_lpi2c_tracedump(struct s32k1xx_lpi2c_priv_s* priv) {
    ...
    for (i = 0; i < priv->tndx; i++) {
        ...
        syslog(LOG_DEBUG,
               ...
               "TIME: %d\n",
               i + 1, trace->status, trace->count, g_trace_names[trace->event], trace->event, trace->parm,
...
static void s32k1xx_lpi2c_setclock(struct s32k1xx_lpi2c_priv_s* priv, uint32_t frequency) {
    ...
    if (priv->config->mode == LPI2C_MASTER) {
        if (frequency != priv->frequency) {
            ...
            men = s32k1xx_lpi2c_getreg(priv, S32K1XX_LPI2C_MCR_OFFSET) & LPI2C_MCR_MEN;
            if (men) {
            ...
            for (uint32_t prescale = 1; (prescale <= 128) && (best_error != 0); prescale *= 2) {
...
static inline void s32k1xx_lpi2c_sendstart(struct s32k1xx_lpi2c_priv_s* priv, uint8_t address) {
    ...
    s32k1xx_lpi2c_modifyreg(priv, S32K1XX_LPI2C_MCFGR1_OFFSET, LPI2C_MCFGR1_IGNACK, 0);

    do {
        txcount = (s32k1xx_lpi2c_getreg(priv, S32K1XX_LPI2C_MFSR_OFFSET) & LPI2C_MFSR_TXCOUNT_MASK) >>
        ...
        if (status & LPI2C_MSR_ERROR_MASK) {
            s32k1xx_lpi2c_putreg(priv, S32K1XX_LPI2C_MSR_OFFSET, status & LPI2C_MSR_ERROR_MASK);
        }
    } while (txcount == 0);
    ...
    s32k1xx_lpi2c_putreg(priv, S32K1XX_LPI2C_MTDR_OFFSET, (LPI2C_MTDR_CMD_START | LPI2C_MTDR_DATA(addr)));
}
 ...
 ****************************************************************************/
#ifdef CONFIG_S32K1XX_LPI2C_DMA
static inline uint32_t s32k1xx_lpi2c_getenabledints(struct s32k1xx_lpi2c_priv_s* priv) {
...
static int s32k1xx_lpi2c_isr_process(struct s32k1xx_lpi2c_priv_s* priv) {
    ...
    if (priv->rxdma != NULL || priv->txdma != NULL) {
        ...
        if ((status & (LPI2C_MSR_SDF | LPI2C_MSR_EPF)) != 0) {
            ...
            s32k1xx_lpi2c_putreg(priv, S32K1XX_LPI2C_MSR_OFFSET, status & (LPI2C_MSR_SDF | LPI2C_MSR_EPF));
        }
        ...
        if (current_status & LPI2C_MSR_ERROR_MASK) {
            ...
            /* Clear the error */
            s32k1xx_lpi2c_putreg(priv, S32K1XX_LPI2C_MSR_OFFSET,
                                 (current_status & (LPI2C_MSR_NDF | LPI2C_MSR_ALF | LPI2C_MSR_FEF)));
    ...
    /* Check if there is more bytes to send */
        ...
        if (priv->dcnt > 0) {
            ...
            s32k1xx_lpi2c_putreg(priv, S32K1XX_LPI2C_MTDR_OFFSET, LPI2C_MTDR_CMD_TXD | LPI2C_MTDR_DATA(*priv->ptr++));
            priv->dcnt--;
    ...
    /* Check if there is more bytes to read */
        ...
        if (priv->dcnt > 0) {
            ...
            /* Receive a byte */
            *priv->ptr++ = s32k1xx_lpi2c_getreg(priv, S32K1XX_LPI2C_MRDR_OFFSET) & LPI2C_MRDR_DATA_MASK;
            priv->dcnt--;
    ...
    if (priv->dcnt <= 0) {
        if (priv->msgc > 0 && priv->msgv != NULL) {
            ...
            if ((priv->flags & I2C_M_READ) != 0) {
                #ifndef CONFIG_I2C_POLLED
                /* Stop TX interrupt */
                s32k1xx_lpi2c_modifyreg(priv, S32K1XX_LPI2C_MIER_OFFSET, LPI2C_MIER_TDIE, LPI2C_MIER_RDIE);
                #endif
                /* Set LPI2C in read mode */
                s32k1xx_lpi2c_putreg(priv, S32K1XX_LPI2C_MTDR_OFFSET,
                                     LPI2C_MTDR_CMD_RXD | LPI2C_MTDR_DATA((priv->dcnt - 1)));
            }
            else {
            ...
            s32k1xx_lpi2c_modifyreg(priv, S32K1XX_LPI2C_MIER_OFFSET, LPI2C_MIER_TDIE | LPI2C_MIER_RDIE, 0);
        }
        #endif
    }
...
static int s32k1xx_lpi2c_init(struct s32k1xx_lpi2c_priv_s* priv) {
    ...
    s32k1xx_lpi2c_modifyreg(priv, S32K1XX_LPI2C_MCFGR0_OFFSET, LPI2C_MCFG0_HREN | LPI2C_MCFG0_HRSEL, LPI2C_MCFG0_HRPOL);

    /* Pin config and ignore NACK disable */
    s32k1xx_lpi2c_modifyreg(priv, S32K1XX_LPI2C_MCFGR1_OFFSET, LPI2C_MCFGR1_IGNACK | LPI2C_MCFGR1_PINCFG_MASK, 0);

    /* Set tx and rx watermarks */
    s32k1xx_lpi2c_putreg(priv, S32K1XX_LPI2C_MFCR_OFFSET, LPI2C_MFCR_TXWATER(0) | LPI2C_MFCR_RXWATER(0));
    ...
    s32k1xx_lpi2c_putreg(priv, S32K1XX_LPI2C_MCFGR3_OFFSET, LPI2C_MCFG3_PINLOW_CYCLES(0));
 ...
 ****************************************************************************/
#ifdef CONFIG_S32K1XX_LPI2C_DMA
static int s32k1xx_lpi2c_dma_command_configure(struct s32k1xx_lpi2c_priv_s* priv, uint16_t* ccmd, uint32_t ncmd) {
    ...
    up_clean_dcache((uintptr_t)config.saddr, (uintptr_t)config.saddr + config.nbytes);
 ...
 ****************************************************************************/
#ifdef CONFIG_S32K1XX_LPI2C_DMA
static int s32k1xx_lpi2c_dma_data_configure(struct s32k1xx_lpi2c_priv_s* priv, struct i2c_msg_s* msg) {
    DMACH_HANDLE                    dma;
    struct s32k1xx_edma_xfrconfig_s config;
    memset(&config, 0, sizeof(config));
    ...
    if (msg->flags & I2C_M_READ) {
        ...
        up_invalidate_dcache((uintptr_t)msg->buffer, (uintptr_t)msg->buffer + msg->length);
    }
    else {
        dma          = priv->txdma;
        ...
        config.doff  = 0;
        up_clean_dcache((uintptr_t)msg->buffer, (uintptr_t)msg->buffer + msg->length);
    }
 ...
 ****************************************************************************/
#ifdef CONFIG_S32K1XX_LPI2C_DMA
static int s32k1xx_lpi2c_form_command_list(struct s32k1xx_lpi2c_priv_s* priv, struct i2c_msg_s* msg, int ncmds) {
    ssize_t length = 0;

    if (priv->flags & I2C_M_NOSTART) {
        if (priv->flags & I2C_M_READ) {
            /* No start read operation */
            priv->cmnds[ncmds++] = LPI2C_MTDR_CMD_RXD | LPI2C_MTDR_DATA(msg->length - 1);
        }
    }
    else {
        ...
        uint16_t badd = (priv->flags & I2C_M_READ) ? I2C_READADDR8(msg->addr) : I2C_WRITEADDR8(msg->addr);
        ...
        if (badd & I2C_READBIT) {
            ...
            while (length) {
                if (length > 256u) {
                    priv->cmnds[ncmds++] = LPI2C_MTDR_CMD_RXD | LPI2C_MTDR_DATA(256u - 1);
                    length -= 256u;
                }
                else {
                    priv->cmnds[ncmds++] = LPI2C_MTDR_CMD_RXD | LPI2C_MTDR_DATA(length - 1);
                    length               = 0;
                }
            }
        }
...
static int s32k1xx_lpi2c_dma_transfer(struct s32k1xx_lpi2c_priv_s* priv) {
    ...
    /* Disable Interrupts */
    s32k1xx_lpi2c_modifyreg(priv, S32K1XX_LPI2C_MIER_OFFSET, LPI2C_MIER_RDIE | LPI2C_MIER_TDIE, 0);

    /* Disable DMA */
    s32k1xx_lpi2c_modifyreg(priv, S32K1XX_LPI2C_MDER_OFFSET, LPI2C_MDER_TDDE | LPI2C_MDER_RDDE, 0);

    /* Turn off auto_stop option */
    s32k1xx_lpi2c_modifyreg(priv, S32K1XX_LPI2C_MCFGR1_OFFSET, 0, LPI2C_MCFGR1_IGNACK | LPI2C_MCFGR1_AUTOSTOP);
    ...
    for (m = 0; m < priv->msgc; m++) {
        ...
        ncmds = s32k1xx_lpi2c_form_command_list(priv, &priv->msgv[m], ntotcmds);
    ...
    s32k1xx_lpi2c_putreg(priv, S32K1XX_LPI2C_MIER_OFFSET,
                         LPI2C_MIER_NDIE | LPI2C_MIER_ALIE | LPI2C_MIER_PLTIE | LPI2C_MIER_FEIE);

    s32k1xx_dmach_start(priv->rxdma, s32k1xx_rxdma_callback, priv);
    s32k1xx_dmach_start(priv->txdma, s32k1xx_txdma_callback, priv);

    s32k1xx_lpi2c_modifyreg(priv, S32K1XX_LPI2C_MDER_OFFSET, 0, LPI2C_MDER_TDDE | LPI2C_MDER_RDDE);
    return OK;
}
#endif
...
struct i2c_master_s* s32k1xx_i2cbus_initialize(int port) {
    ...
    if (priv->refs++ == 0) {
        ...
        if (priv->config->dma_txreqsrc != 0) {
            priv->txdma = s32k1xx_dmach_alloc(priv->config->dma_txreqsrc | DMAMUX_CHCFG_ENBL, 0);
            DEBUGASSERT(priv->txdma != NULL);
        }

        if (priv->config->dma_rxreqsrc != 0) {
            priv->rxdma = s32k1xx_dmach_alloc(priv->config->dma_rxreqsrc | DMAMUX_CHCFG_ENBL, 0);
