...
static void s32k1xx_lpspi_dumpregs(struct s32k1xx_lpspidev_s* priv, char const* msg);
#else
...
static void     s32k1xx_lpspi_setTCR(struct s32k1xx_lpspidev_s* priv, enum spi_slave_mode_e mode, 
                                     uint16_t nbits, uint8_t PCSpin);

/* SPI slave controller methods */

static void s32k1xx_lpspi_bind(struct spi_slave_ctrlr_s* ctrlr, struct spi_slave_dev_s* dev,
                               enum spi_slave_mode_e mode, int nbits);
static void s32k1xx_lpspi_unbind(struct spi_slave_ctrlr_s* ctrlr);
static int  s32k1xx_lpspi_enqueue(struct spi_slave_ctrlr_s* ctrlr, void const* data, size_t len);
...
static int s32k1xx_lpspi_enqueue(struct spi_slave_ctrlr_s* ctrlr, void const* data, size_t len) {
    ...
    for (nwords = 0; nwords < len; nwords++) {
        ...
        if (SPIS_CTRLR_QFULL(ctrlr) == false) {
            priv->outq[priv->tx_head] |= *(uint8_t const*)(data++) << 0;
            priv->outq[priv->tx_head] |= *(uint8_t const*)(data++) << 8;
            priv->outq[priv->tx_head] |= *(uint8_t const*)(data++) << 16;
            priv->outq[priv->tx_head] |= *(uint8_t const*)(data++) << 24;

            spiinfo("enqueue [%d]:%04X %04X", priv->tx_head, (uint16_t)((priv->outq[priv->tx_head] >> 16) & 0xffff),
    ...
    s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_IER_OFFSET, 0, LPSPI_IER_TEIE | LPSPI_IER_TDIE | LPSPI_IER_FCIE);
...
static uint32_t s32k1xx_lpspi_dequeue(struct s32k1xx_lpspidev_s* priv) {
    ...
    if (priv->tx_head != priv->tx_tail) {
        ...
        spiinfo("dequeue [%d]:%04X %04X", priv->tx_tail, (uint16_t)((priv->outq[priv->tx_tail] >> 16) & 0xffff),
        ...
        if (priv->tx_head == next) {
            s32k1xx_lpspi_modifyreg32(priv, 
                                      S32K1XX_LPSPI_IER_OFFSET, LPSPI_IER_TEIE | LPSPI_IER_TDIE | LPSPI_IER_FCIE,
                                      0);
        }
    }
    else {
        ...
        s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_IER_OFFSET, LPSPI_IER_TEIE | LPSPI_IER_TDIE | LPSPI_IER_FCIE, 0);
    }
...
static int s32k1xx_lpspi_interrupt(int irq, void* context, void* arg) {
    ...
    for (;;) {
        ...
        if ((pending & LPSPI_SR_REF) != 0) {
            ...
            s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_IER_OFFSET, LPSPI_IER_RDIE, 0);
            ...
            s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_SR_OFFSET, 0, LPSPI_SR_REF);

            /* Renable interrupt flag again after reset FIFO */
            s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_IER_OFFSET, 0, LPSPI_IER_RDIE);
        }
        ...
        if ((pending & LPSPI_SR_RDF) != 0) {
            ...
            data = (uint32_t)((regval & LPSPI_RDR_DATA_MASK) >> LPSPI_RDR_DATA_SHIFT);

            /* Report the receipt of data to the SPI device driver */
            SPIS_DEV_RECEIVE(priv->dev, (uint32_t const*)&data, sizeof(data));
        ...
        if ((pending & LPSPI_SR_TEF) != 0) {
            ...
            s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_IER_OFFSET, LPSPI_IER_TDIE, 0);

            /* cleared Recieve Error Flag (TEF) flag */
            s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_SR_OFFSET, 0, LPSPI_SR_TEF);

            /* Reenable interrupt flag again after reset FIFO */
            s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_IER_OFFSET, 0, LPSPI_IER_TDIE);
        }
        ...
        if ((pending & LPSPI_SR_FCF) != 0) {
            ...
            s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_IER_OFFSET, LPSPI_IER_TEIE | LPSPI_IER_TDIE, 0);

            /* cleared Frame Complete Flag (FCF) flag */
            s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_SR_OFFSET, 0, LPSPI_SR_FCF);
...
static void s32k1xx_lpspi_bind(struct spi_slave_ctrlr_s* ctrlr, struct spi_slave_dev_s* dev,
    ...
    priv->outval = *(uint32_t const*)data;
    ...
    regval = LPSPI_SR_DMF | LPSPI_SR_REF | LPSPI_SR_TEF | LPSPI_SR_TCF | LPSPI_SR_FCF | LPSPI_SR_WCF;
...
static void s32k1xx_lpspi_unbind(struct spi_slave_ctrlr_s* ctrlr) {
    ...
    s32k1xx_lpspi_putreg32(priv, S32K1XX_LPSPI_CR_OFFSET, LPSPI_CR_RST | LPSPI_CR_RRF | LPSPI_CR_RTF);
    s32k1xx_lpspi_putreg32(priv, S32K1XX_LPSPI_CR_OFFSET, LPSPI_CR_RST | LPSPI_CR_RRF | LPSPI_CR_RTF);
...
struct spi_slave_ctrlr_s* s32k1xx_spi_slave_initialize(int bus) {
    ...
    if (!priv->initialized) {
        ...
        /* Execute a software reset of the SPI (twice) */
        s32k1xx_lpspi_putreg32(priv, S32K1XX_LPSPI_CR_OFFSET, LPSPI_CR_RST | LPSPI_CR_RTF | LPSPI_CR_RRF);
        s32k1xx_lpspi_putreg32(priv, S32K1XX_LPSPI_CR_OFFSET, LPSPI_CR_RST | LPSPI_CR_RTF | LPSPI_CR_RRF);
        s32k1xx_lpspi_putreg32(priv, S32K1XX_LPSPI_CR_OFFSET, 0);
        ...
        s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_CFGR1_OFFSET, LPSPI_CFGR1_MASTER, LPSPI_CFGR1_PINCFG_SOUT_SIN);
        ...
        if (bus == 0) {
            ...
            if ((PIN_LPSPI0_PCS == PIN_LPSPI0_PCS0_1) || (PIN_LPSPI0_PCS == PIN_LPSPI0_PCS0_2) ||
                (PIN_LPSPI0_PCS == PIN_LPSPI0_PCS0_3)) {
                priv->PCSpin = 0;
            }
            else if ((PIN_LPSPI0_PCS == PIN_LPSPI0_PCS1_1) || (PIN_LPSPI0_PCS == PIN_LPSPI0_PCS1_2)) {
                priv->PCSpin = 1;
            }
        ...
        if (bus == 1) {
            ...
            if ((PIN_LPSPI1_PCS == PIN_LPSPI1_PCS0_1) || (PIN_LPSPI1_PCS == PIN_LPSPI1_PCS0_2) ||
                (PIN_LPSPI1_PCS == PIN_LPSPI1_PCS0_3)) {
                priv->PCSpin = 0;
            }
            else if ((PIN_LPSPI1_PCS == PIN_LPSPI1_PCS1_1) || (PIN_LPSPI1_PCS == PIN_LPSPI1_PCS1_2)) {
                priv->PCSpin = 1;
            }
        ...
        if (bus == 2) {
            ...
            if ((PIN_LPSPI2_PCS == PIN_LPSPI2_PCS0_1) || (PIN_LPSPI2_PCS == PIN_LPSPI2_PCS0_2) ||
                (PIN_LPSPI2_PCS == PIN_LPSPI2_PCS0_3) || (PIN_LPSPI2_PCS == PIN_LPSPI2_PCS0_4)) {
                priv->PCSpin = 0;
            }
            else if ((PIN_LPSPI2_PCS == PIN_LPSPI2_PCS1_1) || (PIN_LPSPI2_PCS == PIN_LPSPI2_PCS1_2)) {
                priv->PCSpin = 1;
            }
        ...
        /* Set water masks */
        s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_FCR_OFFSET, LPSPI_FCR_TXWATER_MASK, LPSPI_FCR_TXWATER(0));
        s32k1xx_lpspi_modifyreg32(priv, S32K1XX_LPSPI_FCR_OFFSET, LPSPI_FCR_RXWATER_MASK, LPSPI_FCR_RXWATER(1));

        /* Set Transmit Command Register */
        s32k1xx_lpspi_setTCR(priv, CONFIG_SPI_SLAVE_DRIVER_MODE, CONFIG_SPI_SLAVE_DRIVER_FRAME_SIZE, priv->PCSpin);
