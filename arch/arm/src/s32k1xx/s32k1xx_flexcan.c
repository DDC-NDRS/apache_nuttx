 ...
 *
 ****************************************************************************/
uint32_t s32k1xx_bitratetotimeseg(struct flexcan_timeseg* timeseg, int32_t sp_tolerance, uint32_t can_fd) {
    int32_t tmppresdiv;
    int32_t numtq;
    int32_t tmpsample;
    int32_t tseg1;
    ...
    for (tmppresdiv = 0; tmppresdiv < PRESDIV_MAX; tmppresdiv++) {
        ...
        if ((CLK_FREQ / ((tmppresdiv + 1) * numtq) == timeseg->bitrate) && 
            ...
            if (((tseg1 >= TSEG1MAX) || (tseg2 >= TSEG2MAX) || (tseg2 < TSEG_MIN) || (tseg1 < TSEG_MIN)) ||
                ((tmppropseg >= SEGMAX) || (tmppseg1 >= SEGMAX) || (tmppseg2 < SEG_MIN) || (tmppseg2 >= SEGMAX))) {
                continue;
            }

            tmpsample = ((tseg1 + 1) * 100) / numtq;

            if ((tmpsample - timeseg->samplep) <= sp_tolerance && (timeseg->samplep - tmpsample) <= sp_tolerance) {
                if (can_fd == 1) {
...
static uint32_t s32k1xx_waitmcr_change(uint32_t base, uint32_t mask, uint32_t target_state);

/* Interrupt handling */
static void s32k1xx_receive(struct s32k1xx_driver_s* priv, uint32_t flags);
static void s32k1xx_txdone_work(void* arg);
static void s32k1xx_txdone(struct s32k1xx_driver_s* priv);

static int  s32k1xx_flexcan_interrupt(int irq, void* context, void* arg);
...
static int /**/s32k1xx_transmit(struct s32k1xx_driver_s* priv) {
    ...
    if ((getreg32(priv->base + S32K1XX_CAN_ESR2_OFFSET) & (CAN_ESR2_IMB | CAN_ESR2_VPS)) ==
        (CAN_ESR2_IMB | CAN_ESR2_VPS)) {
        mbi = ((getreg32(priv->base + S32K1XX_CAN_ESR2_OFFSET) & CAN_ESR2_LPTM_MASK) >> CAN_ESR2_LPTM_SHIFT);
        mbi -= RXMBCOUNT;
    }
    ...
    if (priv->dev.d_sndlen > priv->dev.d_len) {
        struct timeval* tv       = (struct timeval*)(priv->dev.d_buf + priv->dev.d_len);
        priv->txmb[mbi].deadline = *tv;
        timeout = (tv->tv_sec - ts.tv_sec) * CLK_TCK + ((tv->tv_usec - ts.tv_nsec / 1000) * CLK_TCK) / 1000000;
        if (timeout < 0) {
            return 0; /* No transmission for you! */
        }
    }
        ...
        if (CONFIG_NET_CAN_RAW_DEFAULT_TX_DEADLINE > 0) {
            timeout                          = ((CONFIG_NET_CAN_RAW_DEFAULT_TX_DEADLINE / 1000000) * CLK_TCK);
            priv->txmb[mbi].deadline.tv_sec  = ts.tv_sec + CONFIG_NET_CAN_RAW_DEFAULT_TX_DEADLINE / 1000000;
    ...
    if (timeout >= 0) {
        wd_start(&priv->txtimeout[mbi], timeout + 1, s32k1xx_txtimeout_expiry, (wdparm_t)priv);
    }
    #endif
 ...
 ****************************************************************************/
static int s32k1xx_txpoll(struct net_driver_s* dev) {
    struct s32k1xx_driver_s* priv = (struct s32k1xx_driver_s*)dev->d_private;
...
static void s32k1xx_receive(struct s32k1xx_driver_s* priv, uint32_t flags) {
    ...
    while ((mb_index = arm_lsb(flags)) != 32) {
        ...
        if (rf->cs.edl) {                   /* CAN FD frame */
            ...
            putreg32(1 << mb_index, priv->base + S32K1XX_CAN_IFLAG1_OFFSET);
...
static void s32k1xx_txtimeout_work(void* arg) {
    ...
    for (mbi = 0; mbi < TXMBCOUNT; mbi++) {
        if (priv->txmb[mbi].deadline.tv_sec != 0 &&
            (now->tv_sec > priv->txmb[mbi].deadline.tv_sec || now->tv_usec > priv->txmb[mbi].deadline.tv_usec)) {
...
static uint32_t s32k1xx_waitmcr_change(uint32_t base, uint32_t mask, uint32_t target_state) {
    ...
    for (wait_ack = 0; wait_ack < timeout; wait_ack++) {
        bool const state = (getreg32(base + S32K1XX_CAN_MCR_OFFSET) & mask) != 0;
...
static uint32_t s32k1xx_waitfreezeack_change(uint32_t base, uint32_t target_state) {
...
static void s32k1xx_reset(struct s32k1xx_driver_s* priv) {
    ...
    regval &= ~CAN_MCR_MAXMB_MASK; /* Zero MAXMB to ensure "bitwise or"
                                    ...
                                    */
    regval |= CAN_MCR_SLFWAK | CAN_MCR_WRNEN | CAN_MCR_SRXDIS | CAN_MCR_IRMQ | CAN_MCR_AEN |
...
int /**/s32k1xx_caninitialize(int intf) {
    ...
    if (priv->config->lprx_irq > 0) {
        if (irq_attach(priv->config->lprx_irq, s32k1xx_flexcan_interrupt, priv)) {
            /* We could not attach the ISR to the interrupt */
    ...
    priv->tx            = (struct mb_s*)(priv->base + S32K1XX_CAN_MB_OFFSET + (sizeof(struct mb_s) * RXMBCOUNT));
