/****************************************************************************
 * boards/arm/s32k1xx/s32k146evb/src/s32k1xx_gpio.c
 *
...
static struct s32k1xx_gpio_dev_s g_gpio[DEV_NUM_OF_GPIO];
#endif
...
static int gpio_read(struct gpio_dev_s* dev, bool* value) {
    ...
    DEBUGASSERT(s32k1xx_gpio->id < DEV_NUM_OF_GPIO);
    gpioinfo("Reading...\n");
...
static int gpio_irqattach(struct gpio_dev_s* dev, pin_interrupt_t callback) {
    ...
    s32k1xx_pinirqattach(g_gpiopins[s32k1xx_gpio->id], s32k1xx_gpio_interrupt, &g_gpio[s32k1xx_gpio->id]);
...
static int gpio_setpintype(struct gpio_dev_s* dev, enum gpio_pintype_e pintype) {
    ...
    pinconfig = (g_gpiopins[s32k1xx_gpio->id] & ~(_PIN_MODE_MASK | _PIN_OPTIONS_MASK | _PIN_INT_MASK));
    ...
    if ((s32k1xx_gpio->gpio.gp_pintype >= GPIO_INTERRUPT_PIN) && (s32k1xx_gpio->gpio.gp_pintype < GPIO_NPINTYPES)) {
