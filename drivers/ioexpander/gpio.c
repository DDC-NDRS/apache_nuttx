...
static ssize_t gpio_write(FAR struct file* filep, FAR char const* buffer, size_t buflen);
...
static int gpio_handler(FAR struct gpio_dev_s* dev, uint8_t pin) {
    ...
    for (i = 0; i < CONFIG_DEV_GPIO_NSIGNALS; i++) {
        ...
        nxsig_notification(signal->gp_pid, &signal->gp_event, SI_QUEUE, &signal->gp_work);
    }
...
static ssize_t /**/gpio_write(FAR struct file* filep, FAR char const* buffer, size_t buflen) {
    ...
    if (dev->gp_pintype != GPIO_OUTPUT_PIN && dev->gp_pintype != GPIO_OUTPUT_PIN_OPENDRAIN) {
        return -EACCES;
    }
        ...
        case GPIOC_REGISTER :
            if (arg && dev->gp_pintype >= GPIO_INTERRUPT_PIN) {
                ...
                if (i == 0) {
                    ...
                    ret = dev->gp_ops->go_attach(dev, (pin_interrupt_t)gpio_handler);
                    if (ret >= 0) {
 ...
 *
 ****************************************************************************/
int gpio_pin_register_byname(FAR struct gpio_dev_s* dev, FAR char const* pinname) {
    char devname[32];
    int  ret;
    ...
    switch (dev->gp_pintype) {
        ...
        case GPIO_OUTPUT_PIN_OPENDRAIN : {
            DEBUGASSERT(dev->gp_ops->go_read != NULL && dev->gp_ops->go_write != NULL);
        } break;

        default : {
            DEBUGASSERT(dev->gp_ops->go_read != NULL && dev->gp_ops->go_attach != NULL &&
 ...
 *   EBUSY  - Ref count is not 0;
 ****************************************************************************/
int gpio_pin_unregister_byname(FAR struct gpio_dev_s* dev, FAR char const* pinname) {
    char devname[32];
