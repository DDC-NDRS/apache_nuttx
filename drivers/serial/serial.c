...
#if defined(CONFIG_SERIAL_IFLOWCONTROL) && defined(CONFIG_SERIAL_IFLOWCONTROL_WATERMARKS)
...
static inline ssize_t uart_irqwrite(FAR uart_dev_t* dev, FAR char const* buffer, size_t buflen);
static int            uart_tcdrain(FAR uart_dev_t* dev, bool cancelable, clock_t timeout);

static int uart_tcsendbreak(FAR uart_dev_t* dev, FAR struct file* filep, unsigned int ms);
...
static ssize_t uart_write(FAR struct file* filep, FAR char const* buffer, size_t buflen);
        ...
        /* Otherwise we are going to have to wait for data to arrive */
            ...
            if (rxbuf->head == rxbuf->tail) {
                ...
                #endif
                {
                    ...
                    if (dev->timeout) {
                        ret = nxsem_tickwait(&dev->recvsem, DSEC2TICK(dev->timeout));
                    }
                    else
                    #endif
                    {
...
#ifdef CONFIG_TTY_LAUNCH_ENTRY
        task_spawn(CONFIG_TTY_LAUNCH_ENTRYNAME, CONFIG_TTY_LAUNCH_ENTRYPOINT, NULL, &attr, argv, NULL);
#else
 ...
 *
 ****************************************************************************/
int uart_register(FAR char const* path, FAR uart_dev_t* dev) {
    #if defined(CONFIG_TTY_SIGINT) || defined(CONFIG_TTY_SIGTSTP)
...
int uart_check_special(FAR uart_dev_t* dev, char const* buf, size_t size) {
    size_t i;
